...
import SingleContentPreview from "containers/PageSingle/SingleContentPreview";
import { SinglePageType } from "containers/PageSingle/PageSingle";

export interface PageSingleGalleryPreviewProps {
  className?: string;
  pageData: SinglePageType,
}

const PageSingleGalleryPreview: FC<PageSingleGalleryPreviewProps> = ({ className = "", pageData }) => {
  ...
  useEffect(() => {
    dispatch(
      changeCurrentPage({ type: "/single/:slug", data: pageData })
    );
    return () => {
  ...
  const PHOTOS = pageData.galleryImgs || [];
...
export default PageSingleGalleryPreview;
